...
🔒 Authentication & Access Control
Authentication Flow
User login/register via /login atau /register.
Backend mengirimkan Bearer Token, profil user, dan data saldo.
Token disimpan di localStorage dan status isAuthenticated menjadi true.
Axios Interceptor (

src/api.js
) secara otomatis menyisipkan token ke setiap header request API berikutnya.
📂 Routing Structure
text
src/
├── pages/
...
Interceptor Pattern: Memastikan keamanan setiap request secara terpusat di 
...
npm run dev
👨‍💻 Author
Akbar Pangestu "Building scalable and real-world fullstack applications."
